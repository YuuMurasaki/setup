...
xbps-install -y dbus dbus-x11 pam_rundir NetworkManager ImageMagick ffmpeg gnupg pinentry-gtk
xbps-install -y pipewire wireplumber rtkit alsa-pipewire
xbps-install -y git oath-toolkit make curl wget patch
xbps-install -y lua luarocks LuaJIT gcc tcc
xbps-install -y stow htop slop zathura-pdf-poppler
xbps-install -y fzf eza trash-cli rsync tar zip unzip 7zip
